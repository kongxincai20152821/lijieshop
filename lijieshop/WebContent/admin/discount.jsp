...
<script type="text/javascript">
  ...
  $(function(){
	  $("#dg").datagrid({
		 idField:"discount_id",
		 url:"../discountQueryByPage",
		 queryParams:formToJson("searchform"),
				 columns:[[
					 {field:"",checkbox:true },
					 {field:"discount_id",width:100,title:"打折ID",align:"center" },
					 {field:"new_price",width:100,title:"现价",align:"center" },
					 {field:"discount_number",width:100,title:"打几折",align:"center" }
		         ]],
					toolbar:[
						{
							text:"新增",
  ...
  function remove(){
		...
		$.messager.confirm('提示信息',"你确认要删除这"+array.length+"条记录？",function(r){
			if(r){
				...
				for(var i=0;i<array.length;i++){
					ids += array[i].discount_id+","
				}
				ids = ids.substring(0,ids.length-1);
				$.ajax({
					url:"../discountDeleteMore",
					type:"post",
					data:"ids="+ids,
					dataType:"json",
					success:function(result){
    ...
    function update(){
        ...
        //表单提交 会获取表单的所有信息
	    $('#ff').form('submit',{
				url:'../discountUpdate?id='+selectedRow.discount_id, //selectedRow.empno 获取选中行的数据id
...
<div style="">
   <form  id="searchform" method="post" style="text-align:center;padding:5px">
		   <a id="research_btn" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-reload'" style="width:130px;position:relative;float:left">清空查询条件</a>
		      打折ID：<input class="easyui-textbox" name="qDiscountId"  style="width:100px"> 
		      打折数：<input class="easyui-textbox" name="qDiscountNumber" style="width:100px"> 
...
<div id="addDialog" class="easyui-dialog" style="top:100px;padding:20px"   data-options="iconCls:'icon-save',resizable:true,modal:true,closed:true"  >
  <form action="" method="post" id="zz" style="">
       <table>
			<tr><td>现价：</td><td><input name="new_price" class="easyui-textbox" data-options="required:true"></td></tr>
			<tr><td>打几折：</td><td><input name="discount_number" class="easyui-textbox" data-options="required:true"></td></tr>
       </table>
  </form>
</div>
<!-- 修改弹窗 -->
