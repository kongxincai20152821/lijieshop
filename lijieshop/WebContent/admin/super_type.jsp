...
<script type="text/javascript">
  ...
  function remove(){
		...
		$.messager.confirm('提示信息',"你确认要删除这"+array.length+"条记录？",function(r){
			if(r){
				...
				for(var i=0;i<array.length;i++){
					ids += array[i].stype_id+","
				}
				ids = ids.substring(0,ids.length-1);
				$.ajax({
					url:"../superTypeDeleteMore",
					type:"post",
					data:"ids="+ids,
					dataType:"json",
					success:function(result){
    ...
    function update(){
        ...
        //表单提交 会获取表单的所有信息
	    $('#ff').form('submit',{
				url:'../superTypeUpdate?id='+selectedRow.stype_id, //selectedRow.empno 获取选中行的数据id
...
<div style="">
   <form  id="searchform" method="post" style="text-align:center;padding:5px">
		   <a id="research_btn" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-reload'" style="width:130px;position:relative;float:left">清空查询条件</a>
		      大分类ID：<input class="easyui-textbox" name="qSuperId"  style="width:100px"> 
		      大分类名称：<input class="easyui-textbox" name="qSuperName" style="width:100px"> 
...
<div id="addDialog" class="easyui-dialog" style="top:100px;padding:20px"   data-options="iconCls:'icon-save',resizable:true,modal:true,closed:true"  >
  <form action="" method="post" id="zz" style="">
       <table>
			<tr><td>名称：</td><td><input name="stype_name" class="easyui-textbox" data-options="required:true"></td></tr>
       </table>
  </form>
</div>
<!-- 修改弹窗 -->
