...
<script type="text/javascript">
  ...
  $(function(){
	  $("#dg").datagrid({
		 idField:"comment_id",
		 url:"../commentQueryByPage",
		 queryParams:formToJson("searchform"),
				 columns:[[
					 {field:"",checkbox:true },
					 {field:"comment_id",width:100,title:"评论ID",align:"center" },
					 ...
					 {field:"title",width:100,title:"评论标题",align:"center" },
					 {field:"content",width:100,title:"评论内容",align:"center" },
					 {field:"publish_time",width:100,title:"发布时间",align:"center" }
		         ]],
					toolbar:[
						{
							text:"新增",
  ...
  function remove(){
		...
		$.messager.confirm('提示信息',"你确认要删除这"+array.length+"条记录？",function(r){
			if(r){
				...
				for(var i=0;i<array.length;i++){
					ids += array[i].comment_id+","
				}
				ids = ids.substring(0,ids.length-1);
				$.ajax({
					url:"../commentDeleteMore",
					type:"post",
					data:"ids="+ids,
					dataType:"json",
					success:function(result){
    ...
    function update(){
        ...
        //表单提交 会获取表单的所有信息
	    $('#ff').form('submit',{
				url:'../commentUpdate?id='+selectedRow.comment_id, //selectedRow.empno 获取选中行的数据id
...
<div style="">
   <form  id="searchform" method="post" style="text-align:center;padding:5px">
		   <a id="research_btn" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-reload'" style="width:130px;position:relative;float:left">清空查询条件</a>
		      评论ID：<input class="easyui-textbox" name="qCommentId"  style="width:100px"> 
...
<div id="addDialog" class="easyui-dialog" style="top:100px;padding:20px"   data-options="iconCls:'icon-save',resizable:true,modal:true,closed:true"  >
  <form action="" method="post" id="zz">
       <table>
			<tr><td>用户ID：</td><td><input name="user_id" class="easyui-numberbox" data-options="required:true"></td></tr>
			<tr><td>商品ID：</td><td><input name="goods_id" class="easyui-numberbox" data-options="required:true"></td></tr>
			<tr><td>评论标题：</td><td><input name="title" class="easyui-textbox" data-options="required:true"></td></tr>
			<tr><td>评论内容：</td><td><input name="content" class="easyui-textbox" data-options="required:true"></td></tr>
			<tr><td>发布时间：</td><td><input name="publish_time" class="easyui-datebox" data-options="required:true"></td></tr>
       </table>
  </form>
</div>
<!-- 修改弹窗 -->
<div id="editDialog" class="easyui-dialog" style="top:100px;padding:20px"   data-options="iconCls:'icon-save',resizable:true,modal:true,closed:true"  >
  <form  method="post" id="ff">
       <table>
			<tr><td>评论ID：</td><td><input name="comment_id" class="easyui-numberbox" data-options="required:true"></td></tr>
			<tr><td>用户ID：</td><td><input name="user_id" class="easyui-numberbox" data-options="required:true"></td></tr>
			<tr><td>商品ID：</td><td><input name="goods_id" class="easyui-numberbox" data-options="required:true"></td></tr>
			<tr><td>评论标题：</td><td><input name="title" class="easyui-textbox" data-options="required:true"></td></tr>
			<tr><td>评论内容：</td><td><input name="content" class="easyui-textbox" data-options="required:true"></td></tr>
