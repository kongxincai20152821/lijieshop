...
<script type="text/javascript">
  ...
  $(function(){
	  $("#dg").datagrid({
		 idField:"ltype_id",
		 url:"../littleTypeQueryByPage",
		 queryParams:formToJson("searchform"),
				 columns:[[
					 {field:"",checkbox:true },
					 {field:"ltype_id",width:100,title:"小分类表ID",align:"center" },
					 {field:"stype_id",width:100,title:"大分类表ID",align:"center" },
					 {field:"stype_name",width:100,title:"大分类名称",align:"center" },
					 {field:"ltype_name",width:100,title:"名称",align:"center" }
		         ]],
					toolbar:[
						{
							text:"新增",
  ...
  function remove(){
		...
		$.messager.confirm('提示信息',"你确认要删除这"+array.length+"条记录？",function(r){
			if(r){
				...
				for(var i=0;i<array.length;i++){
					ids += array[i].ltype_id+","
				}
				ids = ids.substring(0,ids.length-1);
				$.ajax({
					url:"../littleTypeDeleteMore",
					type:"post",
					data:"ids="+ids,
					dataType:"json",
					success:function(result){
    ...
    function update(){
        ...
        //表单提交 会获取表单的所有信息
	    $('#ff').form('submit',{
				url:'../littleTypeUpdate?id='+selectedRow.ltype_id, //selectedRow.empno 获取选中行的数据id
...
<div style="">
   <form  id="searchform" method="post" style="text-align:center;padding:5px">
		   <a id="research_btn" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-reload'" style="width:130px;position:relative;float:left">清空查询条件</a>
		      小分类ID：<input class="easyui-textbox" name="qLittleId"  style="width:100px"> 
		      小分类名称：<input class="easyui-textbox" name="qLittleName" style="width:100px"> 
...
<div id="addDialog" class="easyui-dialog" style="top:100px;padding:20px"   data-options="iconCls:'icon-save',resizable:true,modal:true,closed:true"  >
  <form action="" method="post" id="zz" style="">
       <table>
			<tr><td>大分类ID：</td><td><input name="stype_id" class="easyui-textbox" data-options="required:true"></td></tr>
			<tr><td>名称：</td><td><input name="ltype_name" class="easyui-textbox" data-options="required:true"></td></tr>
       </table>
  </form>
</div>
<!-- 修改弹窗 -->
<div id="editDialog" class="easyui-dialog" style="top:100px;padding:20px"   data-options="iconCls:'icon-save',resizable:true,modal:true,closed:true"  >
  <form  method="post" id="ff">
       <table>
			<tr><td>小分类表ID：</td><td><input name="ltype_id" class="easyui-textbox" data-options="required:true"></td></tr>
			<tr><td>大分类ID：</td><td><input name="stype_id" class="easyui-textbox" data-options="required:true"></td></tr>
			<tr><td>名称：</td><td><input name="ltype_name" class="easyui-textbox" data-options="required:true"></td></tr>
       </table>
  </form>
